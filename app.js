...
function dirClassFrom(dx,dy){return({"0,-1":"face-up","1,-1":"face-up-right","1,0":"face-right","1,1":"face-down-right","0,1":"face-down","-1,1":"face-down-left","-1,0":"face-left","-1,-1":"face-up-left"}[`${dx},${dy}`]||"face-down")}
...
function start(){let name=(E.name.value||"名無し").replace(/[<>]/g,"").trim()||"名無し",du=DUN[E.ds.value]||DUN.beginner;localStorage.md_player_name=name;localStorage.md_dungeon_id=du.id;let carry=JSON.parse(localStorage.md_pending_items_v030||"[]").slice(0,8);localStorage.removeItem("md_pending_items_v030");let carriedGold=baseGold();setBaseGold(0);let prog=loadProgress(du);S={name,du,f:1,t:0,map:[],items:[],en:[],shop:null,st:{x:13,y:13},def:0,end:false,log:[],known:initialKnown(du),canReturn:false,fx:null,heroAnimUntil:0,heroAnimDir:"face-down",face:{dx:0,dy:1},p:{x:1,y:1,hp:prog.mhp,mhp:prog.mhp,lv:prog.lv,ex:prog.ex||0,atk:prog.atk,def:prog.def,food:100,g:du.startG+carriedGold,poison:false,eq:{weapon:null,shield:null,ringR:null,ringL:null,arrow:null},inv:[item("h1",true,false),item("bread",true,false),...carry]}};sel="";floor(true);sortInv();log(`${du.name}に挑戦します。${du.resetLevel?"このダンジョンはレベル1から開始します。":"レベルは前回までの成長を引き継ぎます。"}`);log("装備中アイテムは緑色で :E が付き、整頓時は一番上に並びます。");faceMode=false;updateFaceButton();render();renderWh();loadRank()}
...
function meleeAttack(dx,dy){triggerHeroAttack(dx,dy);let targets=meleeTargets(dx,dy);if(!targets.length)return false;targets.forEach(e=>attackOne(e));return true}
...
function shootSelected(idv=sel){if(!S||S.end)return;let it=equipItem("arrow")||findItem(idv)||S.p.inv.find(x=>x.cat==="arrow");if(!it||it.cat!=="arrow"){log("撃つ矢を装備または所持していません。");render();return}let {dx,dy}=S.face;if(dx===0&&dy===0){dx=0;dy=1}triggerHeroAttack(dx,dy);let hit=null,lastFloor={x:S.p.x,y:S.p.y};for(let r=1;r<=10;r++){let x=S.p.x+dx*r,y=S.p.y+dy*r;if(!inside(x,y)||S.map[y][x]===WALL)break;lastFloor={x,y};hit=S.en.find(e=>e.x===x&&e.y===y);if(hit)break}it.qty--;if(it.qty<=0)removeIt(it.id);if(hit){let dm=it.dmg||8;hit.hp-=dm;effect(hit.x,hit.y);log(`${dn(it,false)}を撃ち、${hit.n}に${dm}ダメージ。`);if(hit.hp<=0)kill(hit)}else{dropArrowAt(it,lastFloor.x,lastFloor.y);effect(lastFloor.x,lastFloor.y);log(`${dn(it,false)}を撃ちましたが外れ、${lastFloor.x},${lastFloor.y}に落ちました。`)}after()}
